...
All applications in the :ref:`Dashboard <dashboard>` are packed in a Docker image and have `rclone <https://rclone.org/>`__ installed by default. If you want to create a Docker containing your own application, you should install rclone in the container to be able to access the data stored remotely. When developing an application with the :doc:`AI Modules Template </reference/modules/template>`, the Dockerfile already includes installation of rclone.
